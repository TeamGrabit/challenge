...
async function FixChallengeInfo(req, res) {
	const challenge_id = req.params.challenge_id;
	const id = ObjectID(challenge_id);
	var { name, challenge_start, challenge_end, challenge_leader, user_id, private_key, vacation_count } = req.body;
	...
	try {
		...
		const changeInfo = () => Challenge.findOneById(id).then((ch) => {
			...
			if (vacation_count === undefined) {
				vacation_count = preChallenge.vacation_count;
			}
		}).then(() => {
